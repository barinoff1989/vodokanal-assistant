...
Backend отвечает за оркестрацию и сборку промпта, а шлюз — за
...
async def test_прямой_ответ_не_вызывает_провайдера():
    """Требование ADR-100: ответ не стоит ни токенов, ни ожидания.
...
async def test_оговорка_доходит_обоими_путями():
    ...
    ответе целиком, пока поле не добавили и в поток.
    """
    backend = Orchestrator(_gateway(completion=_explode), direct=(AlwaysAnswers(),))
...
async def test_отказ_хранилища_на_прямом_пути_даёт_503():
    """ADR-400: недоступность хранилища — это `503`, а не `429`.
...
async def test_контекст_ищется_и_доходит_до_шлюза():
    """Backend ищет, шлюз получает промпт с уже собранным контекстом."""
...
def test_шлюз_не_знает_про_поиск():
    """На диаграмме связь Backend → Vector DB, а не шлюз → Vector DB."""
    import app.gateway.llm_gateway as gateway_module
...
async def test_правила_не_нашли_тему_но_модель_помогла():
    """Живой случай: «отключение» без слова «вода» мимо
...
async def test_тема_из_метаданных_классификатор_не_зовёт():
    """Тема пришла снаружи — ни правила, ни модель её не трогают."""
...
# --- тип обращения уходит в фильтр поиска (ADR-200) ------------------------------ #
