...
Остальные проверки сторожат конкретные решения: семь проблем раздела 7.6 и
...
def test_департамента_в_метаданных_нет(spec: dict):
    """Решение в обратную сторону от того, что предлагал раздел 7.6.
...
def test_все_ошибки_в_формате_rfc7807(spec: dict):
    """Правило 4.5, без исключений для отдельных кодов."""
    responses = spec["paths"]["/v1/generate"]["post"]["responses"]
...
# --- ограничения запроса (раздел 7.2) -------------------------------------------------- #
