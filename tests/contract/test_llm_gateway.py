...
сверх плана — `503` при недоступности хранилища: без неё решение ADR-400
осталось бы текстом.
...
async def test_ответ_содержит_источники():
    """Контракт требует источники; раньше поля принимались, но не возвращались."""
    request = _request(
        context=[
            ContextChunk(
                chunk_id="c1",
...
async def test_у_отказа_по_лимиту_есть_время_повтора():
    """Дефект: заголовок `Retry-After` был пропущен вовсе."""
    gateway = _gateway(
        quota=QuotaManager(FakeRedis(), subscriber_limit=1, session_limit=1, service_limit=1)
    )
...
async def test_лимит_проверяется_до_обращения_к_модели():
    """Порядок: отклонить дешевле, чем обезличивать и вызывать."""
    calls: list[int] = []
...
# --- отказ хранилища, ADR-400 (проверка 3, сверх плана) -------------------------- #
...
async def test_данные_абонента_не_уходят_провайдеру():
    """Правило защиты ПДн, проверенное на том, что реально ушло в вызов."""
    seen: dict[str, Any] = {}
...
async def test_контекст_уходит_модели_как_есть():
    """Обратное прежнему поведению — и это решение ADR-400, а не послабление.
...
async def test_вопрос_абонента_обезличивается_по_прежнему():
    """Отказ от обезличивания базы знаний (ADR-400) сузил правило защиты ПДн, а не отменил его."""
    seen: dict[str, Any] = {}
...
# --- синтетический источник помечается ----------------------------- #
