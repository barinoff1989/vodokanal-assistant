"""Проверки реестра регламентных ответов (ADR-012).
...
def test_неутверждённая_запись_роняет_строгую_загрузку():
    ...
    вопрос на генерацию — туда, откуда ADR-012 его и убирал."""
...
def test_запись_прототипа_помечена_неутверждённой():
    ...
    текст, принятый за утверждённый (журнал, раздел 55.4)."""
...
def test_текст_прототипа_ничего_не_утверждает_за_владельца():
    ...
    водоснабжения, и делать его за владельца нельзя (ADR-012).
