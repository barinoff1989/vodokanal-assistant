...
def test_длинное_число_без_подсказки_не_трогается():
    """Шаблон из девяти-десяти цифр слишком широк сам по себе.
...
def test_значения_не_остаются_в_тексте(sanitizer: PiiSanitizer):
    """Главное требование правила защиты ПДн, проверенное напрямую."""
...
# (136 текстов). До отсева обезличиватель давал на них
...
# состоит настоящее обращение. Это пятый случай одной закономерности и
...
# --- одиночное слово в начале предложения -------------------------- #
