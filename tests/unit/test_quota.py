...
Отдельно проверяется поведение при отказе хранилища: ADR-400 требует отклонять
...
# --- отказ хранилища: ADR-400 ---------------------------------------------------- #


def test_недоступное_хранилище_отклоняет_запрос():
    """ADR-400: не пропускаем. Отказ может быть следствием самого всплеска."""
