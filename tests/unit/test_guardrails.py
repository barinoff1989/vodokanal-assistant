...
Четыре сценария безопасности вынесены в отдельный блок и
...
def test_утечка_отличима_от_прочих_блокировок(guardrails: SyncGuardrails):
    """Дашборд: утечка вынесена в отдельный виджет, а не смешана с общим счётчиком.
...
async def test_поток_обрывается_на_нарушении():
    """Правило потоковой выдачи требует потока, но не ценой показа запрещённого.
