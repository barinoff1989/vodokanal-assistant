...
def test_порог_отсекает_далёкое(tmp_path: Path):
    """Ниже порога контекст считается не найденным.
...
def test_в_промпт_уходит_не_больше_запрошенного(tmp_path: Path):
    """`top_n = 3` (ADR-200). Лишние фрагменты — лишние токены и лишний шум."""
...
# --- переранжирование (ADR-300, app/kb/reranker.py) ---------------------------- #
...
# --- веса берутся из кэша, а не из сети ----------------------------- #
...
# --- вопрос и ответ индексируются порознь --------------------------- #
