...
# --- цепочки запасных провайдеров (пункт 33) --------------------------------------- #
...
def test_у_прототипа_запасной_локальная_модель(settings: Settings):
    """Требование ADR-002: минимум два провайдера с переключением при сбое."""
    chains = fallback_chains(load_config(settings=settings))
...
def test_в_конфигурации_нет_зарубежных_провайдеров():
    """ADR-002: передача персональных данных за периметр исключена.
...
def test_промпты_не_логируются():
    """Тот же кодовый путь используется с внешним провайдером (правило 4.2)."""
    document = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
...
async def test_переключение_на_запасного_провайдера_работает():
    """Требование ADR-002, «Подтверждение», пункт 1.
    ...
    записано в разделе 49.3 контекста.
