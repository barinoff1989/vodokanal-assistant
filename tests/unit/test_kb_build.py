"""Проверки приёма корпуса базы знаний (ADR-015).

Здесь сторожится **гарантия, переехавшая ко входу**. Прежде контекст чистился на
каждом ответе, и это портило текст: «Перерасчёт» превращался в «⟨ФИО⟩». ADR-015
...
def test_обычный_текст_регламента_приём_проходит(body: str):
    ...
    То есть ложные срабатывания, ради избавления от которых ADR-015 и написан,
...
def test_пустой_корпус_приём_проходит():
    """Пустая база знаний — отдельный случай с отдельным решением (пункт 54),
