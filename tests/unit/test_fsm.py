...
**запрещённые не работают**. Правило подтверждения записи держится именно на запретах: если
...
def test_каждый_шаг_пути_попал_в_аудит():
    """Правило подтверждения записи требует аудита на всех путях записи."""
    session = _session(complete_slots=True)
    for event in (
...
# --- правило подтверждения записи: запись только после подтверждения --------------------------- #
...
def test_отказ_абонента_возвращает_к_правке():
    """Сценарий: должна быть возможность отмены или исправления до отправки."""
...
def test_отказ_абонента_попадает_в_аудит():
    """Аудит: отказ пишется в аудит с пометкой, что запись не выполнялась."""
    session = _session(InquiryState.AWAITING_CONFIRMATION)
...
# --- полнота черновика (Draft Validator) ------------------------- #
...
# --- ADR-400: подтверждение решения -------------------------------------------- #


def test_автомат_не_имеет_внешних_зависимостей():
    """Пункт 1 подтверждения ADR-400: проверяется составом импортов.
...
def test_состав_переходов_зафиксирован():
    """Пункт 4 подтверждения ADR-400.

    При миграции на другой исполнитель состав переходов обязан сохраниться:
    таблица — формулировка правила подтверждения записи, а не деталь реализации. Число здесь
