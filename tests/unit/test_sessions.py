"""Проверки состояния диалога (шаг 7).
...
(правило 4.7). Ошибка тут не роняет сервис: она отдаёт чужой черновик или
...
def test_срок_жизни_проставляется(store: SessionStore, redis: FakeRedis):
    ...
    неограниченно (пункт 18 сведённого TODO)."""
...
def test_отказ_хранилища_не_роняет_ответ():
    """Не `503`, в отличие от хранилища лимитов (ADR-008).
    ...
    выполнится. Правило 4.7 остаётся выполненным по построению, а вопрос про
