...
def test_параметры_поиска_совпадают_с_принятыми():
    ...
    **Порог изменён с 0.7 на 0.80** (ADR-014). Прежнее значение раздела 8.2
    взято под другую модель эмбеддингов; на e5-small посторонние вопросы
    набирают 0,74–0,79 и прошли бы насквозь. Тест поймал расхождение при
    правке — ровно за этим он и стоит, — поэтому число здесь меняется вместе с
    решением, а не вслед за настройкой.
    """
    settings = _settings()
