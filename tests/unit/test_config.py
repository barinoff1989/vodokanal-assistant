...
def test_параметры_поиска_совпадают_с_принятыми():
    """Параметры поиска: 20 кандидатов, 3 фрагмента в промпт.
    ...
    **Порог менялся дважды, и оба раза вместе с решением, а не вслед за
    ...
    0,7 → 0,80 (ADR-300): прежнее значение взято под другую модель
...
def test_задано_время_повтора_для_отказа_по_лимиту():
    """Правило ошибок по RFC 7807: код 429 без `Retry-After` недопустим."""
...
def test_локальная_модель_запрещена_в_production():
    """Она не проходила ни оценки качества, ни замера задержки (ADR-300)."""
...
def test_модель_эмбеддингов_задана_настройкой():
    """ADR-300: на прототипе e5-small, на MVP BGE-M3 по ADR-300.
...
def test_число_фрагментов_в_промпте_меньше_числа_кандидатов():
    """`top_k = 20 -> top_n = 3` (ADR-200).

    На прототипе переранжирования между ними нет (ADR-300), но соотношение
