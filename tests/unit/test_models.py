...
def test_непредусмотренное_поле_отклоняется():
    ...
    в ключах словаря из прошлого прототипа.
    """
    with pytest.raises(ValidationError):
...
def test_департамента_в_метаданных_нет():
    ...
    Тест закрепляет отход от прежней редакции, чтобы поле не вернули по невнимательности.
    """
...
def test_текст_запроса_не_попадает_в_печатное_представление():
    """Правило защиты ПДн чаще нарушают отладочным выводом, а не отправкой наружу."""
...
def test_ожидание_подтверждения_распознаётся():
    """Правило подтверждения записи: запись возможна только из этого состояния."""
    state = SessionState(
        session_id="s",
        subscriber_id="sub",
...
def test_обрыв_охранителем_отличим_от_нормального_завершения():
    """Иначе в аналитике блокировка выглядела бы как обычный ответ."""
