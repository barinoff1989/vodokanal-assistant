...
Шесть проверок из карточки шага 9, каждая названа так, чтобы связь была видна.
...
# --- проверка 1: запись только после подтверждения (правило 4.7) ------------- #
...
# --- проверка 5: область записи в Биллинг сужена (ADR-009) --------------------- #
...
# --- проверка 6: одна внешняя система на путь (ADR-009) ------------------------ #
...
def test_авария_не_регистрируется_как_обращение():
    """ADR-009 отменил прежнее утверждение раздела 5.1.
