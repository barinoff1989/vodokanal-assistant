...
BRANCH_PHONE = "8 (473) 206-77-06"
"""Единый контакт-центр воронежского филиала.

Взят из корпуса базы знаний, где стоит в восьми ответах из двадцати и назван
именно так. Это единственный воронежский номер, чьё назначение известно."""

OTHER_BRANCH_PHONES: tuple[str, ...] = (
    "8-861-217-03-33",
    "8-800-200-36-46",
    "8-903-411-86-76",
)
"""Опубликованные номера **других** филиалов, встречающиеся в выгрузке.

ПОЧЕМУ ИХ ВООБЩЕ НАДО ТРОГАТЬ. Сами по себе они настоящие, и правило проекта —
настоящие данные не переписывать. Но в той же строке 87 организация **уже
заменена нами** на «ООО "РВК-Воронеж"»: сочетание «воронежская организация,
краснодарские телефоны» создано этой нормализацией, а не пришло из жизни.
Оставить его — значит оставить артефакт собственной правки.

ПЕРЕЧЕНЬ ЯВНЫЙ, А НЕ ПРАВИЛО «ЛЮБОЙ ЧУЖОЙ НОМЕР». Правило по коду города или по
принципу «всё, чего нет в воронежском списке» однажды переписало бы **личный
номер абонента** — а его надо скрывать, а не подменять справочным. Здесь
перечислены ровно те три, что наблюдались; всё остальное по-прежнему проходит
через обезличиватель и попадает в отчёт.

ЦЕНА ЗАМЕНЫ НАЗВАНА. В исходном тексте три номера отвечают за разное:
информационно-справочная служба (два номера) и автоматизированный приём
показаний. Воронежских соответствий этим службам мы не знаем — в корпусе
известно назначение только у контакт-центра. Поэтому все три сводятся к нему, и
**различение служб теряется**. Догадаться о соответствии было бы хуже: это тот
же класс ошибки, что назначить коды типов обращений по догадке."""
...
_REPEATED_BRANCH_PHONE = re.compile(
    rf"{re.escape(BRANCH_PHONE)}(?:[,;\s]+{re.escape(BRANCH_PHONE)})+"
)
"""Один и тот же номер подряд после замены — свернуть в один.

В исходном тексте перечислены два номера информационно-справочной службы через
запятую. После сведения к единому контакт-центру получалось «по многоканальным
телефонам … 8 (473) 206-77-06, 8 (473) 206-77-06» — список из одного и того же
номера дважды, который читается как опечатка.

Свёртка косметическая и делается **после** замены, а не вместо неё: она не
скрывает потерю различения служб, названную выше, а лишь не добавляет к ней
видимой нелепости."""
...
def to_branch(text: str) -> str:
    ...
    for foreign in OTHER_BRANCH_PHONES:
        text = text.replace(foreign, BRANCH_PHONE)
    text = _REPEATED_BRANCH_PHONE.sub(BRANCH_PHONE, text)
...
def describe(report: Report) -> Iterator[str]:
    ...
    yield f"телефоны других филиалов сведены к {BRANCH_PHONE}"
