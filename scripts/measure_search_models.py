...
ADR-007 фиксирует **BGE-M3** для эмбеддингов и кросс-энкодер
...
меньше расчётной, а время до первого токена лучше на два порядка (раздел 51).

ЧТО ЛЕЖИТ НА СИНХРОННОМ ПУТИ АБОНЕНТА, А ЧТО НЕТ

| Операция | Путь | Бюджет |
|---|---|---|
| Эмбеддинг **запроса** | синхронный | входит в `P99 поиска < 100 мс` (DoD, раздел 18) |
...
переранжирование — на тех самых двадцати кандидатах, что задаёт ADR-006.
...
# выбранная ADR-007 пара не укладывается в бюджет, следующий вопрос — «а что
# укладывается», и линейка для ответа должна быть той же самой.

# Бюджеты, с которыми сравниваются числа (DoD, раздел 18).
...
def measure_embedder(
    ...
    # неожиданностью (разделы 50.3, 51.1, 54.5).
    model.encode([QUERY])
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"(DoD, раздел 18), время до первого токена — {TTFT_BUDGET_MS:.0f} мс.\n"
