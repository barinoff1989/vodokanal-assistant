...
отсутствие, просроченная поверка) и вручную воспроизвести проверки привязки сессии к абоненту:
войти одним абонентом и попробовать подтвердить черновик другого.

Поэтому файл кладётся в `web/` рядом со стендом, а не в `app/`, и в контракт API ничего не
добавляет: стенд читает его как статический файл.
