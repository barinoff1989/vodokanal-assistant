...
def check_stream(auth: str) -> None:
    """Правило потоковой выдачи требует потока. Проверяем, что он действительно поток."""
    payload = {
        "model": MODEL,
        "messages": [
            {
    ...
    # и лишь потом отдал его частями, а это не поток в смысле правила потоковой выдачи.
    ...
    if incremental and first_at is not None:
        ...
        report(
            ...
            "Нарушает правило потоковой выдачи (ответ доходит до абонента потоком).",
        )
