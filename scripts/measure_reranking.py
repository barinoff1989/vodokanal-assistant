"""Офлайн-замер кросс-энкодера на эталонном наборе (пункт 50 сведённого TODO).

Замер отвечает на **два** вопроса, и второй появился позже первого.

ВОПРОС ПЕРВЫЙ — РАСХОЖДЕНИЕ ТРОЕК

ADR-014 снял переранжирование с пути абонента: кросс-энкодер стоит 5,8 с на
...
всегда — переранжирование не нужно и на MVP, и ADR-007 подлежит пересмотру по
...
Замер порога (журнал, раздел 65.3) показал, что косинусная близость **не
...
иначе. Если его оценка разделяет — у пункта 62 (что делать с порогом) появляется
