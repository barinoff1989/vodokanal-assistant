"""Ночной прогон судьи качества по эталонному набору (код-шаг 10).
...
Пишет СРЕДНИЕ по прогону и долю ответов выше стартовых порогов раздела 36.1
в таблицу `quality_reports` базы `telemetry` — это `quality_report` раздела 35.1
(на MVP ClickHouse, на прототипе Postgres).

СУДЬЯ ≠ ОТВЕЧАЮЩАЯ МОДЕЛЬ (раздел 36.4, ADR-007). Если псевдоним судьи
...
`skipped = true` и причиной, а не отсутствие записи (ADR-007, п. 9).
...
def golden_set_version() -> str:
    ...
    версии набора (раздел 36.5), а поля версии в файле нет."""
