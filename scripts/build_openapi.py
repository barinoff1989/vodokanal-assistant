...
ЧТО ЗАКРЫВАЕТ ЭТА СПЕЦИФИКАЦИЯ (раздел 7.6 контекста, семь проблем):
...
Плюс пятое решение: код `503` при недоступности хранилища лимитов (ADR-008),
которого в разделе 7.4 не было.
...
ERROR_CODES: dict[int, tuple[str, str]] = {
    ...
    429: (
        "Превышена квота",
        "Лимит на абонента, сессию или сервис исчерпан (раздел 35.1). "
        "Заголовок `Retry-After` обязателен.",
    ),
    502: ("Ошибка провайдера", "Провайдер модели ответил ошибкой."),
    503: (
        "Сервис временно недоступен",
        "Недоступно хранилище счётчиков лимитов (ADR-008). "
...
def build() -> dict[str, Any]:
    ...
    spec: dict[str, Any] = {
        ...
        "info": {
            ...
            "description": (
                ...
                "Разрешены **ровно два адреса** (правило 4.4): генерация и проверка "
        ...
        # Проблема 1 раздела 7.6: в прежней редакции рабочим контуром числился
        ...
        "paths": {
            "/v1/generate": {
                "post": {
                    ...
                    "description": (
                        "По умолчанию ответ отдаётся **потоком** (правило 4.3): абонент "
    ...
    # события ошибки не имела вовсе (проблема 3 раздела 7.6).
