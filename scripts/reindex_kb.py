...
запускаемый Celery Beat по расписанию, ночью — `C4_Deployment`) и держит Backend только читающей
стороной. На прототипе своей
...
переживают рестарт (замерено: около 95 секунд на 51 записи без ускорителя). С этим скриптом Backend
при
...
пересборка при каждом старте и есть штатный путь (ADR-100, таблица
упрощённых замен).
