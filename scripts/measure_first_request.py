"""Замер первого запроса после перезапуска по слагаемым.
...
замера нельзя — правило прежних замеров, где ровно такой разбор по
...
тяжёлое из ленивого, 7,0 с по прежнему замеру. Поэтому столбец «старт сервиса
