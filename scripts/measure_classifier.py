...
недоступна (раздел 6.1, шаг 3). До получения выгрузки перечень слов был
...
1. **У точности есть потолок, и задаёт его не классификатор.** «Отмена
   ...
   никаким способом.
2. **«Другое» — свалка** (раздел 56.4): под ним лежат обращения, у которых есть
...
(пункт 14 сведённого TODO), собранном ручной разметкой, а не выгрузкой как есть.
...
необезличенные персональные данные (раздел 56.1); читаем по номеру, а не по
...
class Report:
    ...
    total: int = 0
    dumped: int = 0
    """Строк с типом «Другое» — исключены из точности (раздел 56.4)."""
