...
# Это и есть «переключение одной строкой» из ADR-200: остальной код не меняется.
...
# --- YandexGPT (генерация на прототипе, судья на MVP) -----------------
...
# --- Локальные слепки систем (docker-compose.yml) ----------------------
...
# --- Счётчики лимитов и состояние сессии (docker-compose.yml) ---------
export REDIS_HOST=localhost
export REDIS_PORT=6379
export REDIS_DB=0

# --- Векторное хранилище базы знаний ----------------------------------
...
# бюджете поиска 100 мс (замерено, ADR-300), неработоспособно на синхронном
