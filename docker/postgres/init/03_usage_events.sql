...
--
-- ЧТО ЭТА ТАБЛИЦА ЕСТЬ. Роль «Billing Callback» диаграммы C4_L3_LLM: на MVP
-- события пишутся в ClickHouse (раздел 35.1, 31), на прототипе ClickHouse не
-- развёрнут — его место занимает эта таблица. Prometheus при этом остаётся:
-- он для наблюдаемости в реальном времени (дашборд, раздел 37), а usage_events
-- — фактовая таблица для произвольной нарезки (расход по абоненту/типу/модели,
-- прогноз OPEX, накопление против триггера №2 ADR-002), чего счётчики
...
-- сущностей (из PiiReport, где значений не бывает по построению, правило 4.2).
--
...
CREATE TABLE IF NOT EXISTS usage_events (
    ...
    -- Оси классификации (ADR-011). Код, а не отображаемое название.
    inquiry_type      TEXT,
    topic             TEXT,

    -- Псевдоним — что запросили; model — что фактически ответило. Расхождение
    -- и есть срабатывание запасного провайдера (ADR-002, «Подтверждение», п. 2).
