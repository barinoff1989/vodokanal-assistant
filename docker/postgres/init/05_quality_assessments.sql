...
-- как поток дошёл до абонента (правило 4.3, оценка не блокирует выдачу). Нужна
...
-- различает (пункт 82 TODO): поле provisional = true стоит на каждой строке,
...
CREATE TABLE IF NOT EXISTS quality_assessments (
    ...
    -- Две оценки раздела 36.1, [0, 1]. NULL, если судья не вернул оценку.
    faithfulness      DOUBLE PRECISION,
    answer_relevancy  DOUBLE PRECISION,

    -- Оценка получена слабым прототипным судьёй — не настоящая (пункт 82 TODO).
    provisional       BOOLEAN     NOT NULL DEFAULT true,

    -- scored | unavailable | conflict — почему оценки может не быть:
    -- unavailable — судья не ответил или ответ не разобрать;
    -- conflict — судья совпал с отвечающей моделью (failover, ADR-007).
