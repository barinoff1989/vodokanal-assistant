/*
  ...
  контракт требует POST с телом запроса.
*/
...
// воспроизвести проверку привязки сессии: подтвердить черновик от чужого имени.
//
...
function selectSubscriber(account, { announce } = { announce: true }) {
  ...
  if (announce && changed) {
    ...
    // расхождении проверяется защита от подмены — попытка подтвердить черновик
    // или продолжить сбор бланка от имени абонента, который его не начинал
    // (`SubscriberMismatch`, риски Spoofing/IDOR). Очистка ленты
...
function renderSources(node, sources, disclaimer) {
  ...
  if (listed && listed.length) {
    ...
    const items = sources.map((s) => {
      ...
      // по настоящему регламенту водоканала (ADR-100 предостерегает от
...
/*
  ...
  HITL из правила подтверждения записи: случайное нажатие на подсказку под ответом не должно
...
function showConfirmation() {
  ...
  // правила подтверждения записи, что и молчаливое согласие: подтверждением признаётся
...
ui.confirmYes.addEventListener("click", () => {
  ...
  // эти слова: правило подтверждения записи не признаёт подтверждением свободный текст.
    ...
    const response = await fetch("/v1/generate", {
      ...
      body: JSON.stringify({
        ...
        metadata: {
          ...
          // только по нему решает, выполнять ли запись.
          ...
          channel: "lk_web",
          // Адрес стенд НЕ отправляет: он принадлежит Биллингу, а не запросу.
          // Сервер берёт его сам по идентификатору абонента
          // (`Orchestrator._with_address`) — клиент мог бы прислать чужой и
          // получить график отключений по нему. Профиль слева стенд показывает
          // из того же примера Биллинга, из которого сервер и прочитает.
        },
      }),
    });
    ...
    while (true) {
      ...
      for (const part of parts) {
        ...
        if (event.name === "token") {
          ...
          // с пометкой «учёт позже»: считалось, что провайдер не отдаёт
...
ui.form.addEventListener("submit", (event) => {
  ...
  // "правило подтверждения записи не признаёт подтверждением свободный текст" чуть ниже.
