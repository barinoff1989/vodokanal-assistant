...
Тема `template` (ADR-011) уводит вопрос сюда. Абонент просит бланк, чтобы
...
никуда не пишет: собирает поля и отдаёт готовый текст. Правило 4.7, гейты HITL и
...
опираться на нормативную базу (тот же довод, что у ADR-012). Вопросы к абоненту
