...
последнее звено пути документа на прототипе.
...
* :class:`ArtifactStore` — **дубль объектного хранилища**. На MVP готовые
  документы лежат в S3/MinIO (`app/adapters/document_generator.py`);
  здесь — каталог процесса, а «ссылка с ограниченным сроком жизни» —
...
довод, что у срока жизни состояния сессии. Просроченный файл
