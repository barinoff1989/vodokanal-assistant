...
атрибут отключения и типизированное время (ADR-100), — и тогда этот модуль
...
заводится на адрес, они не выражаются, а файл на MVP исчезнет (ADR-100).
...
class Outage:
    ...
    вслед за источником при переходе на MVP (ADR-100).
    """
