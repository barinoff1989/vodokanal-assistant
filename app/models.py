...
class Intent(StrEnum):
    """Явное действие абонента над черновиком обращения (правило 4.7).
    ...
    **Намерение приходит признаком, а не выводится из текста реплики.** Слова
    ...
    Правило 4.7 требует **явного** подтверждения, а вывод из свободного текста
    явным не бывает.
    ...
    моделью (журнал, раздел 62.2): звать модель, чтобы узнать, разрешена ли
...
class RequestMetadata(BaseModel):
    ...
    черновиком, и это единственный способ дойти до записи: правило 4.7 не
...
class ProblemDetail(BaseModel):
    ...
    """Через сколько секунд повторять — для `429` и `503` (правило 4.5, ADR-008).
...
class DoneEvent(BaseModel):
    ...
    ответе целиком (раздел 7.3); здесь оно появляется, чтобы поток отдавал то же
