...
Отступление **не предрешает** переход на LangGraph (решение и триггеры — ADR-200,
раздел «Оркестрация агента»): наоборот, отсутствие привязки к какой-либо библиотеке делает
