...
`Topic.GENERAL` без вопросов. Хуже, чем сейчас, не станет: в худшем случае —
та же задержка, что и у нынешнего провала (правила молчат → пустой поиск →
отказ консультанта); в лучшем — абонент получает верный путь вместо отказа.
Живой пример — раздел 90 журнала: «По моему адресу до скольки отключение?»
не подошла ни под один маркер `TOPIC_MARKERS['outage']`.
...
без GPU вызов не бесплатен, поэтому — таймаут (`topic_fallback_timeout_seconds`)
и вызов только на подмножестве, а не на каждом запросе (см. выше).
