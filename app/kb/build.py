...
class PersonalDataInCorpusError(RuntimeError):
    """В документе базы знаний нашлись персональные данные (ADR-400).
    ...
    **Ошибка, а не предупреждение.** Предупреждение в журнале — способ не
    ...
    знаний), и заводить вторую нельзя. Документ с персональными
    ...
    def __init__(self, chunk_id: str, entities: tuple[str, ...]) -> None:
        super().__init__(
            ...
            f"{', '.join(entities)}. Документ в базу знаний не берётся (ADR-400). "
...
def check_no_personal_data(items: list[KbItem]) -> None:
    ...
    **Отказ вызывают только сильные свидетельства**, и это не послабление, а
    ...
    избавления от которых принято решение ADR-400, вернулись бы на другом конце:
    ...
    разбор языка ложных находок не давал. Настоящий лицевой счёт
    ...
    **Чего проверка не поймает:** ФИО в тексте документа. Это принятая цена, и
    ...
    а другого пути в корпус нет. Это и есть условие, при котором решение ADR-400 остаётся
...
def collect_items(settings: Settings | None = None) -> list[KbItem]:
    ...
    поиск; на MVP так работать нельзя.
    """
...
просто без своего планировщика задач: на
