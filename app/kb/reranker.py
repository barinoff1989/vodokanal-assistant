"""Переранжирование топ-k кандидатов кросс-энкодером (ADR-007), опционально.

**Выключено по умолчанию, и это не временная деталь.** Замер ADR-014 на машине
...
по-прежнему делается по **косинусной** оценке (0.872, ADR-014), не по оценке
кросс-энкодера: у неё другая шкала (логит, не косинус в [0,1]), и
калиброванного порога для неё в проекте нет — TODO 62 приводит цену такого
варианта («отсекать оценкой кросс-энкодера»), но не фиксирует число. Выдумывать
его здесь значило бы нарушить то же правило, из-за которого писан ADR-014.
"""
...
class CrossEncoderReranker:
    ...
    векторами — отсюда и качество выше, и цена дороже (ADR-007, ADR-014).
    """
    ...
    def warm_up(self) -> None:
        ...
        по следам одной и той же ошибки (разделы 50.3, 51.1, 54.5, ADR-014)."""
        self._ensure()
