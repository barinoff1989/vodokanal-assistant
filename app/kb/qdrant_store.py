"""Qdrant как хранилище векторов базы знаний (ADR-200, минимальная реализация).
...
созданы по `inquiry_type` и `source_title` — по остальным полям ADR-200
(`department`, `effective_date`) данных в корпусе пока нет.

ЧЕГО ЗДЕСЬ НЕТ — ОСОЗНАННО, ЭТО НЕ ВСЁ, ЧТО ОПИСАНО В ADR-200. Проверка несовпадения
версии модели эмбеддингов с алертом (ADR-200) не написана —
отдельная задача дорожной карты (`AI_docs/ADR/ADR-600-roadmap-and-migration.md`), а не забытая
часть этой.
...
# 24 сентября 2026 (10 000 точек, payload, gRPC): выдача 300 точек — P50 19 мс, 500 —
# 29 мс (P99 37), 1 000 — 54 мс; ≈0,05 мс на точку (REST ≈0,016 мс). Граница 300 —
# выбор автора по P50 не выше ≈20 мс (ADR-200, «Значения допущений хранилища»).
EXACT_SCAN_MAX_POINTS = 300
...
# одной пары на фрагмент. Замер 24 сентября 2026 на синтетическом корпусе (50 000
# точек, тематические группы по 8 фрагментов, 5/10/20 точек на фрагмент): при 10
# вся группа в выдаче у 40 из 40 запросов при любом числе точек на фрагмент, при 5 —
# у 37 из 40 (20 точек на фрагмент); топ-3 совпал с полным перебором во всех случаях;
# цена 10 вместо 5 — ≈2 мс (ADR-200, «Значения допущений хранилища»). Реальное число
# точек на фрагмент не измерено. Лучшая точка каждого фрагмента идёт первой, поэтому
# верх выдачи (в промпт идут первые `top_n`) точен; хвост из `top_k` кандидатов может
# оказаться короче.
PARTS_HEADROOM = 10
...
class QdrantVectorStore:
    ...
    :param blue_green: переиндексация без простоя поиска (ADR-200, раздел
    ...
    def upsert(self, entries: Sequence[_Entry]) -> None:
        ...
        if not client.collection_exists(target):
            ...
            client.create_collection(
                ...
                # m/ef — параметры ADR-200, не выдумка модуля.
                hnsw_config=HnswConfigDiff(m=16, ef_construct=128),
            )
            # Индексы payload (ADR-200): без них фильтр — проход
    ...
    def search_parts(
        ...
        *,
        ...
        if self._point_count == 0:
            return []
        client = self._ensure_client()
        # Фильтр — внутри запроса, а не над готовой выдачей (ADR-200): тип
