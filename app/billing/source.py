...
ОТСТУПЛЕНИЕ ОТ ADR-400, ЗАПИСАННОЕ ЯВНО

ADR-400 требует обращаться к чужим системам только через их API и никогда — в их
базу или файлы. На прототипе API нет: закрытая сеть, доступа не будет ни на
разработке, ни на демонстрации. Читать нечего, кроме примера.

Это **второе отступление того же рода**, что у графика отключений (ADR-100), и
...
Запись появится на Этапе 2 через `app/adapters/billing_adapter.py`
