...
Тема `account` (ADR-300) уводит вопрос сюда. Ответ собирается из данных
...
**Четвёртый случай одного правила** после регламентной формулировки (ADR-100),
графика отключений (ADR-100) и тарифов: база знаний отвечает на то, что
