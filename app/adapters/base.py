...
* **Подтверждение абонента** (правило 4.7) — запись возможна ровно из состояния
...
* **Привязка абонента к сессии** — закрывает угрозы Spoofing и IDOR (раздел 13).
...
ОДНА ВНЕШНЯЯ СИСТЕМА НА ПУТЬ (ADR-009).
...
ТОЛЬКО ЧЕРЕЗ API ВЛАДЕЛЬЦА (ADR-003).
...
class ConfirmationRequiredError(RuntimeError):
    """Запись запрошена вне состояния ожидания подтверждения (правило 4.7)."""

    def __init__(self, state: InquiryState) -> None:
        super().__init__(
            f"запись невозможна из состояния {state.value!r}: "
            "правило 4.7 требует подтверждения абонента"
        )
        self.state = state
...
class WriteAdapter:
    ...
    :param system: имя внешней системы. Одно на адаптер — см. ADR-009.
    """
