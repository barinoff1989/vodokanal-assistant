...
ОБЛАСТЬ ЗАПИСИ СУЖЕНА ДО ПОКАЗАНИЙ (ADR-100).
...
ТОЛЬКО ЧЕРЕЗ XML-RPC ВЛАДЕЛЬЦА (ADR-400).
...
> систем ещё не получены (запрошены у владельца).
...
"""Единственная разрешённая операция записи (ADR-100).
...
class UnsupportedBillingOperationError(RuntimeError):
    ...
    def __init__(self, operation: str) -> None:
        super().__init__(
            f"операция {operation!r} не входит в область записи Биллинга (ADR-100); "
...
class BillingAdapter(WriteAdapter):
    ...
    def fetch_account(self, account_number: str) -> dict[str, Any]:
        ...
        операций записи, а показать баланс — обычное действие
        Этапа 1.
        """
    ...
    def submit_meter_reading(
        ...
        Единственная операция записи в Биллинг (ADR-100). Проходит все гейты
    ...
    def write(self, operation: str, *args: Any, **kwargs: Any) -> WriteResult:
        ...
        Существует затем, чтобы попытка добавить операцию мимо решения ADR-100
