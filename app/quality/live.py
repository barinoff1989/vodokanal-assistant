...
ЧТО ЭТО. После того как поток ответа дошёл до абонента (правило потоковой выдачи, оценка не
блокирует выдачу), судья оценивает ответ: следует ли он из найденного контекста
(faithfulness) и отвечает ли на вопрос (answer_relevancy). Итог —
...
слабо различает. Каждая строка помечена `provisional=True`:
...
class LiveJudge:
    ...
    :param answering_alias: псевдоним отвечающей модели — для проверки «судья
        совпал с отвечающей» (ADR-300). Обычно `settings.llm_provider`.
    """
