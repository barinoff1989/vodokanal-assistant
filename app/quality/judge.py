"""Судья качества ответа (LLM-judge, ADR-300 роль 4a).
...
Обе метрики reference-free: эталонный ответ не нужен. Context
...
Правило ADR-300: судья не совпадает с отвечающей моделью, иначе
...
пропускается и факт пропуска фиксируется (ADR-300).

ВЕРСИЯ СУДЬИ ФИКСИРУЕТСЯ.
:class:`QualityScores` несёт `judge_model` — имя модели, которое вернул
провайдер. Без него оценки двух прогонов несравнимы.
...
class JudgeConflict(JudgeError):
    ...
    завышенной и неотличимой от честной, поэтому прогон пропускается (ADR-300).
    """
...
class QualityScores:
    ...
    def passes(self, *, faithfulness_min: float, relevancy_min: float) -> bool:
        """Прошёл ли ответ стартовые пороги."""
        return (
...
class AnswerJudge:
    ...
    def conflicts_with(self, answering_alias: str) -> bool:
        ...
        «та же модель или другая» (ADR-300). Псевдоним отвечавшей модели прогон
