...
_SYSTEM = (
    "Ты — оценщик качества ответов справочного помощника водоканала. "
    "Ты не отвечаешь абоненту и не исправляешь ответ, а только оцениваешь его. "
    "Отвечай строго одним объектом JSON без пояснений вокруг."
)

_RUBRIC = """\
Оцени ОТВЕТ по двум критериям, каждый — число от 0.0 до 1.0.

1. faithfulness — насколько ОТВЕТ следует из КОНТЕКСТА.
   1.0 — каждое утверждение ответа подтверждено контекстом.
   0.5 — часть утверждений не подтверждена, но и не противоречит.
   0.0 — ответ содержит факты, которых в контексте нет, или противоречит ему.

2. answer_relevancy — насколько ОТВЕТ отвечает именно на ВОПРОС.
   1.0 — прямо и полно отвечает на заданный вопрос.
   0.5 — отвечает частично или с лишним.
   0.0 — не отвечает на вопрос или уходит в сторону.

Отказ по регламенту («обратитесь в контакт-центр», когда в контексте ответа
нет) — это faithfulness 1.0; answer_relevancy тем ниже, чем полнее контекст
позволял ответить.
...
Верни JSON вида:
{{"faithfulness": 0.0, "answer_relevancy": 0.0, "reasoning": "одно предложение"}}
"""
