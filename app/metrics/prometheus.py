...
Контракт разрешает ровно два адреса, и `/metrics` среди них нет. Добавить его
...
# время до первого куска — 500 мс, полный ответ — 3 секунды.
...
"""Поиска ещё нет, метрика объявлена заранее и держит ноль.
...
def serve(port: int = 9100) -> None:
    ...
    Отдельный порт, а не адрес в основном приложении: контракт разрешает ровно
...
"""Бюджет — **менее 50 мс**: проверка стоит на пути ответа и
...
«переключались ли», и на вопрос «на кого». Требование ADR-200 —
измерять ежемесячно.
"""
