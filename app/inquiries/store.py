...
REST API (ADR-100). На прототипе микросервиса нет: он внутри закрытой сети, и
описания интерфейса тоже нет. Его место занимает
таблица в Postgres.
...
REST владельца, и всё, что выше адаптера — гейты правила подтверждения записи, привязка к сессии,
...
ПОЧЕМУ ЭТО НЕ НАРУШАЕТ ADR-400
