...
(один root-токен, всё в памяти, без TLS) — необязательный локальный вариант,
а не замена контроля доступа на прототипе. На MVP секреты приходят переменными окружения из Kubernetes Secret,
который создаёт pipeline из GitLab CI variables (ADR-400); этот модуль там не нужен.
"""
