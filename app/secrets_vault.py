...
CI, прод и любое окружение без Vault читают `.env`/переменные окружения как
раньше, без единой лишней строчки кода.

**Dev-режим Vault сам по себе не безопаснее `.env`** (один root-токен, всё в
памяти, без TLS) — это репетиция интеграции для MVP (там — Vault Agent
Injector, ADR по инфраструктуре не написан, раздел 8.5 контекста), а не замена
контроля доступа на прототипе.
"""
...
def load_into_environ() -> None:
    ...
    CI-секреты и явный `.env` всегда важнее Vault. Любая ошибка (Vault не
    поднят, сети нет, секрета нет, токена нет) — предупреждение в лог и работа
    дальше без него, тем же приёмом, что остальной проект обходится без
    недоступных опциональных зависимостей (`_build_usage`, `_build_live_judge`).
    """
