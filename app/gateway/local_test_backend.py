...
Назначение: дать возможность гонять промпты,
...
управляемый API на территории РФ (ADR-200).
...
и внешней модели, а не публичный контракт `/v1/generate` из спецификации API:
тот принадлежит уровню выше и собирается шлюзом. Формулировка требования
...
правилом защиты ПДн. В журнал уходят только имя модели и статистика токенов.
"""
...
class LocalTestBackend:
    ...
    def ensure_allowed(self) -> None:
        ...
        if self.app_env == "production":
            raise LocalBackendForbidden(
                ...
                "генерацию выполняет управляемый API на территории РФ (ADR-200). "
    ...
    async def complete(
        ...
        *,
        ...
        # В журнал — только модель и токены. Ни промпта, ни ответа.
        logger.info(
    ...
    async def stream(
        ...
        *,
        ...
        Поток обязателен на всём пути до абонента, поэтому
