...
ЧТО ЭТО ЧИНИТ.
Принцип проекта — «config.yaml, не код»: смена провайдера должна
...
План допускал оба пути. Прокси — это второй процесс со
...
исполнял. Требование ADR-200 — шлюз с минимум двумя
провайдерами и автоматическим переключением — выполнялось наполовину, и это
было честно записано. Теперь исполняется.
