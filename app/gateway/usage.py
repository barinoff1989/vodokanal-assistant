...
ЧТО ЭТО И ЗАЧЕМ. На MVP каждый вызов модели пишется в ClickHouse (раздел 35.1):
токены, модель, latency, исход — для аналитики расхода, прогноза OPEX и
накопления против триггера №2 ADR-002. На прототипе **ClickHouse не развёрнут**,
и его роль занимает таблица `usage_events` в Postgres. Prometheus при этом
остаётся: он для наблюдаемости в реальном времени (дашборд, раздел 37), а эта
...
что у `SessionStore.save` (раздел 77.5).
...
(из `PiiReport`, где значений не бывает по построению, правило 4.2).
"""
