...
ЧТО ЭТО И ЗАЧЕМ. Каждый вызов модели пишется в таблицу `usage_events` базы
`telemetry` в Postgres: токены, модель, latency, исход — для аналитики расхода,
прогноза OPEX и накопления против триггера №2 ADR-200. ClickHouse не нужен:
по объёму (до нескольких сотен тысяч строк в год на тираже) Postgres хватает,
расчёт и замер — в ADR-200. Prometheus при этом остаётся: он для наблюдаемости в
реальном времени (дашборд), а эта таблица — фактовая, для произвольной
нарезки (по абоненту, типу, модели), чего счётчики Prometheus без взрыва кардинальности не дают.
...
куска не входит. На MVP это станет асинхронной пачкой в ту же базу —
...
class UsageStore:
    """Запись событий использования в Postgres.
