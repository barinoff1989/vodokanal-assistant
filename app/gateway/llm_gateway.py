...
Ради этого компонента в ADR-200 выбран вариант с единым шлюзом: смена провайдера
...
сначала тратить процессор на обезличивание. Правило защиты ПДн при этом не нарушается —
...
ПОТОК ОБЯЗАТЕЛЕН. Основной способ вызова — :meth:`stream`, отдающий
...
===== ============================================================
  ...
  503  хранилище счётчиков недоступно (ADR-400)
...
class LlmGateway:
    ...
    :param live_judge: судья на живом пути — async-часть Guardrails диаграммы
        ...
        Зовётся фоновой задачей после стрима: правило потоковой выдачи не нарушено.
    """
    ...
    def _spawn_judge(
        ...
        Правило потоковой выдачи не нарушено: задача создаётся, когда абонент уже получил
    ...
    def _prepare(self, request: GenerateRequest) -> tuple[list[dict[str, str]], PiiReport]:
        ...
        **Контекст из базы знаний не обезличивается (ADR-400).** Прежняя
        ...
        Тот же приём: проверки не отменены, а сдвинуты ко входу.
    ...
    def _check_quota(self, request: GenerateRequest, trace_id: str) -> ProblemDetail | None:
        ...
        try:
            ...
            # ADR-400: отказ хранилища — это 503, а не 429. Код 429 означал бы
    ...
    def _sources(request: GenerateRequest) -> list[SourceRef]:
        ...
        Контракт требует, чтобы ответ содержал источники; поля принимались на
        входе, но не возвращались. Здесь это исправлено.
        """
        return [
            SourceRef(
    ...
    async def _call(
        ...
        *,
        ...
        if stream:
            ...
            # (правило потоковой выдачи требует потока). Проверено на обоих провайдерах:
    ...
    def _router(self) -> Any:
        ...
        берутся цепочки запасных провайдеров: требование ADR-200 о переключении
    ...
    async def stream(self, request: GenerateRequest) -> AsyncIterator[GatewayEvent]:
        """Отдать ответ потоком — основной путь абонента.

        События идут в том же порядке, что описан в контракте API: куски текста,
        ...
        # триггер перехода на собственную модель (ADR-300) и прогноз OPEX считались по ним же.
        metrics.record_tokens(
            prompt=usage.prompt_tokens, completion=usage.completion_tokens
        )
        ...
        # задачей: абонент ответ уже получил, правило потоковой выдачи не нарушено.
