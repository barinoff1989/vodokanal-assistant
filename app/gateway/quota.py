...
Задаются три предела; при превышении отвечать нужно `429` с
...
чат-интерфейса, и эта ошибка уже была исправлена.

ПОВЕДЕНИЕ ПРИ ОТКАЗЕ ХРАНИЛИЩА — ADR-400.
...
растущая болезненно при масштабировании.
...
class QuotaStoreUnavailableError(RuntimeError):
    ...
    Отдельный тип, а не обычная ошибка подключения: по ADR-400 это состояние
...
class QuotaManager:
    ...
    def check(self, subscriber_id: str, session_id: str) -> QuotaDecision:
        ...
        :raises QuotaStoreUnavailableError: хранилище недоступно (ADR-400).
        """
    ...
    def store_retry_after(self) -> int:
        """`Retry-After` для случая недоступности хранилища (ADR-400)."""
