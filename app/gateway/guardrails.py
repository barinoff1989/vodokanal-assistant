...
def check_groundedness(answer: str, context: Iterable[str], *, threshold: float = 0.3) -> (
    ...
    > контексте. Это не оценка обоснованности в смысле раздела 36 — настоящую
    > делает судья (`app/quality/judge.py`, код-шаг 10): отдельная модель,
    > локальная на прототипе и YandexGPT на MVP (ADR-007). Судья работает вне
    ...
    > оценку судьи — та же дисциплина, что с непроверенными числами в
    > разделах 26.4 и 28.4. У :class:`app.quality.QualityScores` от судьи это
    > поле всегда `False`.
    """
